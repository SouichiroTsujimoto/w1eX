...
const parser = P.createLanguage({
    ...
    CurlyBracesTexts: (r) => P.seq(P.regexp(/\s*\{\s*/), r.Text.many().tieWith(""), P.regexp(/\s*\}\s*/)).map(([lc, content, rc]) => `\n${content}\n`),
    CurlyBracesSentence: (r) => P.seq(P.regexp(/\s*\{\s*/), r.Sentence, P.regexp(/\s*\}\s*/)).map(([lc, content, rc]) => `<div class="section">\n${content}\n</div>`),
